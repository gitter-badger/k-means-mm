...
function choose_init_method(callback) {
    var choices = [{name: "I'll Choose", choice: "user", txtpos_x: -13.2},
                   ...
                   {name: "Predefined", choice: "frankpredefined", txtpos_x: -13.6}
                  ];
...
function reassign_points() {
    for(var j = 0; j < data.length; j++){
        ...
        data[j].distanceToCentroid =  dist(data[j], centroids[ibest]);
        data[j].isOutlier = false;
    }

    
    // shouldLookForOutliers = true;
    
    findOutliers(data,lookForOutliers);
    computeTotalError(lookForOutliers);

    svg.selectAll(".dot")
    .transition()
    .style("fill", function(d) { theColor = 0; if(d.isOutlier) { return 'cyan'; } else { theColor = d.cluster }
...
function findOutliers(someData, lookForOutliers) {

    if(!lookForOutliers) return;

    sortedData = _.sortBy(data,'distanceToCentroid'); // sorts 'data' by distance to centroid
    lastLPointsOfSortedData = _.last(sortedData, totalOutliers);
    someDataPoint = _.where(data, {id: 1}); // delete this later, only to prove we can retrieve a point

    for(var j = 0; j < lastLPointsOfSortedData.length; j++){
        var point = lastLPointsOfSortedData[j];
        data[point.id].isOutlier = true;  // mark a point as an outlier 
    }    
}

function computeTotalError(lookForOutliers) {
    var total = 0;

    for(var i = 0; i < data.length; i++){

        if(lookForOutliers){            
            if(data[i].isOutlier == false) {
                total = total + Math.pow(data[i].distanceToCentroid,2);   // d(x | C)^2 in Sanjay's notation    
            }                        
        } else {
            total = total + Math.pow(data[i].distanceToCentroid,2);   // d(x | C)^2 in Sanjay's notation    
        }
    ...
    $('#totalError').fadeTo('fast',0.3);
    $('#totalError').fadeTo('slow',1);
    return total;
}
...
function updateKCount(){    
    $('#kCount').html(centroids.length);    
}

function updateOutlierCount() {
    outlierPercentage = 0.1;
    totalOutliers = Math.round(data.length * 0.1);    
    $('#outlierCount').html(totalOutliers);
}

function updateXCount() {
    $('#xCount').html(data.length);
}
...
function get_centroids() {
    ...
    if (kmeans_init == "random") {
    ...
    } else if (kmeans_init == "frankpredefined") {

        function frank_predefined() {
            updateKCount();
            centcount += 1;

            /* Choose first centroid is predfined */
            if (centcount == 1) {                
                // centroids[1] = {x: 6.527682369391556, y: -3.4587710843745603, cluster: 1}; //predefined -> gives 221.12397900220896 for E(X,C,L)
                centroids[1] = {x: -2.181052393215572, y: 3.8730416968712786, cluster: 1}; //predefined -> gives  224.7095390628116 for E(X,C,L)                
            } else {
                /* Choose rest as furthest from others */
                var best_length = 0;
                var best_centroid = null;
        ...
        add_next_centroid_button(frank_predefined);
